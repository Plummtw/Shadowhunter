        ...
        GameProcessLock.get_lock(room_id).synchronized {
          ...
          roomround_box match {
            case Full(roomround) if (roomround.round_no.is == 0) =>
              ...
              RoomActor ! NewMessage(room_id, talk)
    ...
    if (room.status.is == RoomStatusEnum.WAITING.toString) {
      val userentry_box =
        for {
          ...
          //talk.save()
          RoomActor ! NewMessage(room.id.is, talk)
          
          // Update 使用者狀態
          DB.use(DefaultConnectionIdentifier) { conn =>
            DB.prepareStatement("update UserEntry set room_flags = '', damaged='0' where room_id = ?", conn) { stmt =>
          ...
          //talk2.save()
          RoomActor ! NewMessage(room.id.is, talk2)
